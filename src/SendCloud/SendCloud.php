...
class SendCloud
{
    ...
    public function sendCommon(Mail $mail)
    {
        ...
        if ($mail->hasAttachment()) {
            ...
            $resonse = $this->client->mutilpost('POST', static::$config [$this->version] ['send'], $bodyData ['body'], $bodyData ['header']);

        } else {
            $param = $this->wrapParam($mail);

            $resonse = $this->client->post($method, static::$config [$this->version] ['send'], '', $param);
            //echo $resonse->body();
        }
    }
    ...
    public function sendTemplate(Mail $mail)
    {
        ...
        if ($mail->hasAttachment()) {
            $bodyData = $this->wrapBody($mail);
            $resonse = $this->client->mutilpost('POST', static::$config [$this->version] ['sendTemplate'], $bodyData ['body'], $bodyData ['header']);
            //echo $resonse->body();
        } else {
            $param = $this->wrapParam($mail);
            $resonse = $this->client->post($method, static::$config [$this->version] ['sendTemplate'], '', $param);
            //echo $resonse->body();
        }
    }
}
